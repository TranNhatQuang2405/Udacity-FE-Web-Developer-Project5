...
export const getWeatherInfo = async (lat, lon, date) => {
    try {
        let res = await fetch(`${weatherBitBaseURL}?lat=${lat}&lon=${lon}&key=${weatherBitApiKey}`)
        let result = await res.json()
...
export const getImageInfo = async (country) => {
    try {
        let countryImagesRes = await fetch(`${pixabayBaseURL}?key=${pixabayKey}&q=${country}&orientation=horizontal&category=place&safesearch=true`)
...
export const deleteTrip = async (geonameId, dateSubmit) => {
    try {
        let res = await fetch(`${serverBaseURL}/trip?geonameId=${geonameId}&dateSubmit=${dateSubmit}`, {
            method: "DELETE",
            headers: {
                "Content-Type": "application/json"
            },
